...
class StatisticInfo {
  ...
  std::unordered_set<TrackingStatistic *> AllStats;
...
public:
  ...
  void registerStatistic(TrackingStatistic *S) { AllStats.insert(S); }
...
void llvm::DisableStatistics() { Enabled = false; }
...
void StatisticInfo::reset() {
  ...
  // Clear the enabled list and release the lock once we're done. Any
